...
# NEW: Warning logic
def get_total_by_category(username, month):
    c.execute("SELECT category, SUM(amount) FROM expenses WHERE username = ? AND strftime('%m', date) = ? GROUP BY category", 
              (username, month))
    return c.fetchall()
...
if not st.session_state.logged_in:
    ...
    if page == "Add Expense":
        ...
        if not df.empty:
            ...
            # 🚨 Warning Display
            spending_by_category = get_total_by_category(st.session_state.username, month_num)
            for cat, amt in spending_by_category:
                limit = limits.get(cat)
                if limit and amt > limit:
                    st.warning(f"🚨 Warning: You are overspending in **{cat}**! You've spent ₹{amt:.2f} (Limit: ₹{limit})")
